from arvados_cwl_tester import *


arvados_project_uuid("ardev-j7d0g-x8khmrfng3nc1uo")


def test_my_first_step_message_one():
    result = arvados_run(
        "./my_first_step.cwl",
        {
            "input_file": {"class": "File", "path": "./test/data/hello_world.txt"},
            "message": "Hello World!",
        },
    )
    assert "hello_world.txt" in result.files
    assert result.files["hello_world.txt"]["size"] > 0
    assert result.command == ["/bin/sh", "-c", "echo 'Hello World!' > hello_world.txt"]


def test_my_first_step_message_two():
    result = arvados_run(
        "./my_first_step.cwl",
        {
            "input_file": VARIABLES["file_2"],
            "message": "Hi CWL!"
        }
    )
    assert "hello_cwl.txt" in result.files
    assert result.files["hello_cwl.txt"]["size"] > 0
    assert result.command == ["/bin/sh", "-c", "echo 'Hi CWL!' > hello_cwl.txt"]


def test_my_first_workflow_defaults():
    result = arvados_run("./my_first_workflow.cwl", {"file": VARIABLES["file_1"]})
    assert "hello_world.txt" in result.files


def test_my_first_workflow_provided_messages():
    result = arvados_run(
        "./my_first_workflow.cwl",
        {
            "file": VARIABLES["file_1"],
            "first_message": "I am testing workflow!",
            "second_message": "My workflow is working!",
        },
        project_uuid="ardev-j7d0g-x8khmrfng3nc1uo",
    )
    assert "hello_world.txt" in result.files
